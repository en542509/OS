...
int main( int argc, char **argv ) {
  ...
  if( ( argc < 5 ) || ( sscanf( argv[TICK_ARG], "%d", &ticks ) != 1 ) || 
      ...
      ( cores < 1 ) || ( sscanf( argv[MSIZE_ARG], "%d", &size ) != 1 ) ||
      ( size < 2 ) || 
      ( size > 64)) {
    fprintf( stderr, "usage: xsim <ticks> <obj file> <cores> <men size>\n" );
    fprintf( stderr, 
    ...
    fprintf( stderr, "      <mem size> is the size of memory in the system (2<= size <= 64)\n" );
    return 1;
  } 
  ...
  if( !xmem_init( size * 1024 ) ) {  // init memory with para size
